...
class Array(Expr):
    ...
    def evaluate(self, psstacks):
        "TO-DO (part2)"
        preIndex = len(psstacks.opstack)
        pre = psstacks.opstack.copy()
        # print('og val', self.value)
        for element in self.value:
            element.evaluate(psstacks)
        temp = psstacks.opstack[preIndex:]
        psstacks.opstack = pre
        psstacks.opPush(ArrayValue(temp))
...
class Name(Expr):
    ...
    def evaluate(self, psstacks):
        if self.var_name[0] == '/':
            ...
            if self.var_name in psstacks.builtin_operators.keys():
